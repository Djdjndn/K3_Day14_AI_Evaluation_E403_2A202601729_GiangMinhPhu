...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for idx, is_relevant in enumerate(relevant_flags, start=1):
            if is_relevant:
                running_relevant += 1
                precision_sum += running_relevant / idx
...
class LLMJudge:
    ...
    def score_response(
        ...
        try:
            ...
            scores = {
                key: max(0.0, min(1.0, float(parsed.get(key, 0.5))))
                for key in rubric
            }
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if "first" in scores and "second" in scores:
                ...
                second_scores.append(float(scores["second"]))
            elif "answer_a" in scores and "answer_b" in scores:
                first_scores.append(float(scores["answer_a"]))
                second_scores.append(float(scores["answer_b"]))
        avg = sum(all_scores) / len(all_scores) if all_scores else 0.0
        positional = False
        if first_scores and second_scores:
            positional = (sum(first_scores) / len(first_scores)) > (sum(second_scores) / len(second_scores)) + 0.1
        return {
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, failure in enumerate(failures, start=1):
            suggestion = suggestions[idx - 1] if idx - 1 < len(suggestions) else "Review this failure case"
            lines.append(
                f"| F{idx:03d} | {failure.failure_type or 'unknown'} | "
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0):
            suggestions.append("Implement hallucination checks to filter unsupported claims")
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append("Improve prompt clarity and add examples that answer the user intent directly")
        if categories.get("incomplete", 0):
            suggestions.append("Increase chunk size/top-k or add few-shot examples for complete answers")
        if len(suggestions) < 3:
            suggestions.append("Review low-score cases manually and expand the golden dataset around repeated failures")
        if len(suggestions) < 3:
            suggestions.append("Tune retrieval ranking to reduce noisy chunks and surface evidence earlier")
        if len(suggestions) < 3:
            suggestions.append("Add regression tests for fixed failures before the next prompt or retrieval change")
        return suggestions
